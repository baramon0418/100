...
for r in range(240) :
    for c in range(240) :
        if img_8[r,c] == -1 :  #값이 지정되지 않은 pixel    #90/3*8 = 240  
            if math.ceil(r / 8 * 3) < 90 and math.ceil( c / 8 * 3) < 90:
                ...
                q1=v1*(1-rat_x)+v2*(rat_x) #올림한 거에서 원본을 빼면 원본에서 버림한 거를 뺀거를 더하면 1이 된다
                q2=v3*(1-rat_x)+v4*(rat_x)
                img_8[r, c] = round(q1*(1-rat_y)+ (q2*(rat_y)))
            else:
                q1=v1*(1-rat_x)+v2*(rat_x) 
                q2=v3*(1-rat_x)+v4*(rat_x)
                img_8[r, c] = round(q1*(1-rat_y)+ (q2*(rat_y)))    
