...
if(empty($page)){
    ...
    $sql="SELECT * FROM post WHERE title like '%$search_input%'";
    ...
    if($total == 0){
        $subboardtype = "$search_input 와(과) 일치하는 검색결과가 없습니다";
    }
    else{
        $subboardtype = "$search_input 의 검색결과 입니다";
    }
		...
		<ul>
          ...
          <?php
            $search_title="%".$search_input."%";
              $sql="SELECT * FROM post WHERE title like '$search_title' ORDER BY time desc limit $start,$finish";
              $result = mysqli_query($connect,$sql);
              $cnt = 0;
              while($row=mysqli_fetch_array($result)){
                ...
                <li>
                    <ul>
                        ...
                        <li><?php
                            $now = DateTime::createFromFormat('U.u', $row['time']+32400);
